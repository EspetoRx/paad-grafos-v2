<template>
  <div>
    <ModalComponent
      :disabled="bsModalDisabled"
      :title="bsModalTitle"
      :bodyComponent="bsModalBody"
      @modal-close="modalClose"
      @modal-return-value="setModalReturnValue"
      :showClose="bsModalShowClose"
    >
    </ModalComponent>
    <ToastContainer :newToast="newToast"></ToastContainer>
    <top-navbar
      @toggle-off-canvas="toggleOffCanvas"
      @offcanvas-for-vis-configure="offCanvasForVisConfigure"
      @offcanvas-for-vis-physics="offCanvasForVisPhysics"
      @offcanvas-for-vis-nodes="offCanvasForVisNodes"
      @offcanvas-for-vis-edges="offCanvasForVisEdges"
      @offcanvas-for-vis-edges2="offCanvasForVisEdges2"
      @offcanvas-for-vis-interactions="offcanvasForVisInteractions"
      @offcanvas-for-vis-layout="offCanvasForVisLayout"
      @offcanvas-for-vis-manipulation="offcanvasForVisManipulation"
    >
    </top-navbar>
    <graph
      :nodes="nodes"
    ...
    <off-canvas
      ...
      @toggle-off-canvas="this.toggleOffCanvas"
      @options-has-changed="this.optionsHasChanged"
      :realNodes="this.nodes"
      :bsModalReturnValue="bsModalReturnValue"
      @nodes-has-changed="this.nodesHasChanged"
      @canvas-key-change="onCanvasKeyChange"
      @send-toast="emitNewToast"
      @open-bs-modal="enableModal"
      @edges-has-changed="edgesHasChanged"
      :realEdges="this.edges"
    ></off-canvas>
  </div>
</template>
<script>
...
export default {
  data() {
    return {
      ...
      edges: [
        { from: 1, to: 3, label: "<b>Aresta 1</b>", value: 1 },
        { from: 1, to: 2, label: "<b><i>Aresta 2</i></b>", value: 2 },
        { from: 2, to: 4, label: "<i>Aresta 3</i>", value: 3 },
        { from: 2, to: 5, label: "<code>Aresta 4</code>", value: 4 },
        { from: 3, to: 3, label: "Aresta 5", value: 5 },
      ],
      options: null,
  ...
  methods: {
    ...
    offCanvasForVisConfigure() {
      this.offCanvas.title =
        "<i class='fa-solid fa-screwdriver-wrench'></i> Vis Js. Configure";
    ...
    offCanvasForVisEdges2() {
      this.offCanvas.title = "<i class='fa-solid fa-code-branch'></i> Vis Js. Arestas";
      this.offCanvas.type = "visjs-edges2";
    },

    offcanvasForVisInteractions() {
      this.offCanvas.title =
        "<i class='fa-solid fa-computer-mouse'></i> Vis Js. Interações";
    ...
    canvasStart(container, graph_data, options) {
      let localNetwork = new Network(container, graph_data, options);
  ...
  mounted() {
    ...
    this.options.nodes.image = new URL(
      `/public/images/paad_logo.png`,
      import.meta.url
    ).href;
    this.options.nodes.brokenImage = new URL(
      `/public/images/paad_logo.png`,
      import.meta.url
    ).href;
