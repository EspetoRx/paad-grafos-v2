<template>
    ...
    ></InputGroupWithLabel> -->
    <div v-if="checkboxValue" class="alert alert-danger mt-1 p-1 mb-0"><i class="fa-solid fa-circle-exclamation"></i> Cuidado: Alterações redesenham o canvas!</div>
    <InputRange v-if="checkboxValue" :inputId="'min-range'" :isLabelEnabled="true" :min="1" :max="max" :step="1" :initialValue="min"
            :labelValue="'Mínimo'" :tooltip="'Options.Edges.Scaling.Min - Se as arestas tiverem um valor, suas larguras serão determinadas pelo valor, pela função de escala e pelos valores mínimo e máximo. O valor mínimo é o valor mínimo permitido.'"
            @update-value-from-range-input="changeMin"
    ></InputRange>
    <InputRange v-if="checkboxValue" :inputId="'max-range'" :isLabelEnabled="true" :min="min" :max="100" :step="1" :initialValue="max"
            :labelValue="'Máximo'" :tooltip="'Options.Edges.Scaling.Max - Esta é a largura máxima permitida quando as arestas são dimensionadas usando a opção de valor.'"
            @update-value-from-range-input="changeMax"
    ></InputRange>
    <Accordion v-if="checkboxValue":accordionData="accordionData" @message="message" @open-bs-modal="openBsModalFunction" :bsModalReturnValue="bsModalReturnValue"
    ></Accordion>
    <!-- <InputTextArea
        v-if="checkboxValue"
